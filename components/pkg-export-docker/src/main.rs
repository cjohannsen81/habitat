...
fn start(ui: &mut UI) -> Result<()> {
    ...
    let default_url = henv::var(DEPOT_URL_ENVVAR).unwrap_or(DEFAULT_DEPOT_URL.to_string());
    ...
    let spec = BuildSpec {
        ...
        url: m.value_of("DEPOT_URL").unwrap_or(&default_url),
        channel: m.value_of("CHANNEL").unwrap_or(&default_channel),
        base_pkgs_url: m.value_of("BASE_PKGS_DEPOT_URL").unwrap_or(&default_url),
...
fn cli<'a, 'b>() -> App<'a, 'b> {
    ...
    clap_app!((name) =>
        ...
        (@arg DEPOT_URL: --url -u +takes_value {valid_url}
            "Install packages from the specified Depot \
        ...
        (@arg CHANNEL: --channel -c +takes_value
            ...
            (default: stable)")
        (@arg BASE_PKGS_DEPOT_URL: --("base-pkgs-url") +takes_value {valid_url}
            "Install base packages from the specified Depot \
            (default: https://bldr.habitat.sh/v1/depot)")
